    ...
    return (
      <div className="grid grid-cols-2">
        <p className="text-xs">
          ...
          <span className="font-medium">{data.location[0]}</span>
        </p>
        <p className="text-xs">
          <span className="font-semibold text-gray-600">Kinh độ: </span>
          <span className="font-medium">{data.location[1]}</span>
        </p>
      </div>
    );
  };
  ...
  const renderName = () => {
    return (
      <div className="flex items-center gap-2">
        ...
        <span className="line-clamp-2 text-sm font-semibold">{data.name}</span>
      </div>
    );
  };
...
const AirQualityCard: React.FC<AirQualityCardProps> = ({ data }) => {
  const formattedTime = data.time ? data.time.split("-").reverse().join("/") : "";
    ...
    <div
      ...
      <div className="space-y-4 p-4">
        ...
        <div className="rounded-lg bg-white/10 p-2 text-center">
          <p className="font-semibold tracking-wide">{data.status || ""}</p>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-6">
            <div className="text-center">
              ...
              <p className="mt-1 text-xl font-bold">{data.aqi_index}</p>
            </div>

            <div className="text-center">
              <h4 className={cn("text-xs font-medium", isYellow ? "text-gray-600" : "text-white/70")}>PM2.5 (µg/m3)</h4>
              <p className="mt-1 text-xl font-bold">{data.pm_25}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <Card className={cn("w-full rounded-lg border border-gray-200 hover:shadow-md", className)} {...props}>
        <div className="flex w-full flex-row items-center justify-between">
          <div className="flex h-full flex-col">
            <p className="text-4xl font-bold text-gray-800">{props.data.temperature.avg}&#8451;</p>
            <p className="mt-1 text-sm text-gray-600">Wind: {props.data.wind_speed}m/s</p>
          </div>
          <div className="flex h-full flex-col items-center">
            <IoCloudy size={32} className="mb-1 text-blue-500" />
            <p className="text-sm font-medium">{props.data.weather}</p>
            <div className="mt-1 flex flex-row gap-2">
              <p className="text-xs text-orange-500">H: {props.data.temperature.max}&#8451;</p>
              <p className="text-xs text-blue-400">L: {props.data.temperature.min}&#8451;</p>
            </div>
          </div>
        </div>
      </Card>
